function fit = fit_meta_d_mcmc_group_correl(nR_S1, nR_S2, mcmc_params, s, fncdf, fninv)
% fit = fit_meta_d_mcmc_correl(nR_S1, nR_S2, mcmc_params, s, fncdf, fninv)
%
% Estimate Bayesian correlation between two conditions across subjects
%
...
monitorparams = {'mu_logMratio','lambda_logMratio','Mratio','r','cS1','cS2'};
...
% Package group-level output
    ...
    fit.mu_Mratio(1) = exp(stats.mean.mu_logMratio(1));
    fit.mu_Mratio(2) = exp(stats.mean.mu_logMratio(2));
    fit.mu_logMratio(1) = stats.mean.mu_logMratio(1);
    fit.mu_logMratio(2) = stats.mean.mu_logMratio(2);
    fit.lambda_logMratio(1) = stats.mean.lambda_logMratio(1);
    fit.lambda_logMratio(2) = stats.mean.lambda_logMratio(2);
